...
from surreal_collage.log import Channel, logger, root_logger
...
def main():
  parser = argparse.ArgumentParser('surreal_collage test runner')
  parser.add_argument(
    '--no-discover', '-n',
    action = 'store_true',
  ...
  parser.add_argument(
    '--log',
    default=SurrealCollageTestCase.output_test_path() / 'log_tests.txt',
    type=Path,
    help='Path to log file'
  )
  parser.add_argument(
    '--xml',
    default=SurrealCollageTestCase.output_test_path() / 'log_tests.xml',
  ...
  if args.verbose:
    logger.console_handler.setLevel(Channel.INFO)
  else:
    logger.console_handler.setLevel(Channel.WARNING)
  ...
  if not args.no_tiny:
    config_file = SurrealCollageTestCase.root_dir / 'tools' / 'tiny.yaml'
    tiny_main([
      'pull',
      '--num-retry', '10', # try 10 times.
      '--retry-wait', '1', # wait for 1 second after failure
      '--num-sim-jobs', '20', # how many files to process at once
  ...
  try:
    ...
    raise  # don't interfere with sys.exit()
